            ...
            <div class="card mb-3">
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        <!-- Avatar -->
                        @php
                            $userAvatar = strtoupper(substr($post->user->name, 0, 2));
                        @endphp
                        <div class="avatar avatar-l me-2">
                            <span class="avatar-initial rounded-circle bg-info">{{$userAvatar}}</span>
                        </div>
                        <!-- User Info -->
                        <div>
                    ...
                    @php
                        $postTime = \Carbon\Carbon::parse($post->updated_at)->diffForHumans();
                    @endphp
                    <small class="text-muted">Last Updated {{$postTime}}</small>
                    @php
                    ...
                    <div class="chat-history-footer shadow-sm">
                        <form class="form-send-message d-flex justify-content-between align-items-center" method="POST"
                            action="{{ url('/comments') }}" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="post_id" value="{{ $post->id }}">
                            <input class="form-control message-input border-0 me-3 shadow-none" type="text"
                                name="comment" placeholder="Type your message here" />
